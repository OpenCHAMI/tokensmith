...
import (
	...
	"github.com/openchami/tokensmith/internal/storage/ent/annotation"
...
func FromEntResource(ctx context.Context, entResource *ent.Resource) (interface{}, error) {
	...
	case "BootstrapTokenPolicy":
		resource := &v1.BootstrapTokenPolicy{
		...
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for BootstrapTokenPolicy: %w", err)
		}
		...
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for BootstrapTokenPolicy: %w", err)
			}
		}
	...
	case "RefreshTokenFamily":
		resource := &v1.RefreshTokenFamily{
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for RefreshTokenFamily: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for RefreshTokenFamily: %w", err)
			}
		}
	...
	case "TokenHierarchy":
		...
		// Unmarshal Spec
		if err := json.Unmarshal(entResource.Spec, &resource.Spec); err != nil {
			return nil, fmt.Errorf("failed to unmarshal spec for TokenHierarchy: %w", err)
		}

		// Unmarshal Status
		if len(entResource.Status) > 0 && string(entResource.Status) != "null" {
			if err := json.Unmarshal(entResource.Status, &resource.Status); err != nil {
				return nil, fmt.Errorf("failed to unmarshal status for TokenHierarchy: %w", err)
			}
		}
